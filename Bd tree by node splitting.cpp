...
bptr search(bptr h,int k,int &ind)
{
	if(h==NULL) 
	return h;
	else
	if(h->arr[h->count-1]<k)
	return search(h->ptr[h->count],k,ind);
	else
	for(int i=0;i<h->count;i++)
	{
		if(h->arr[i]==k)
		...
		if(h->arr[i]>k)
		return search(h->ptr[i],k,ind);
	}
}
int del_key(bptr &h,int k,int d)
{
	int ind=-1;
	bptr temp;
	int ans;
...
int main()
{
	...
	while(in!=-1)
	{
		temp=search(h,in,ind);
		if(temp!=NULL) cout<<"found at "<<ind<<endl;
		else
		cout<<"not found"<<endl;
		cout<<"enter search ";
		cin>>in;
